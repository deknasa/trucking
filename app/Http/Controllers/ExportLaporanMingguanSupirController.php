...
class ExportLaporanMingguanSupirController extends MyController
{
    ...
    public function export4($data, $dari, $sampai, $tradodari_id, $tradosampai_id, $tradodari, $tradosampai)
    {
        ...
        $sheet->mergeCells("R$detail_table_header_row:U$detail_table_header_row");
        $sheet->setCellValue("R" . ($detail_table_header_row + 1), 'Borongan')->getStyle("R2")->applyFromArray($styleHeader)->getFont()->setSize(14)->setBold(true);
        $sheet->mergeCells("R" . ($detail_table_header_row + 1) . ":R" . ($detail_table_header_row + 2));
        ...
        $sheet->mergeCells("T" . ($detail_table_header_row + 1) . ":T" . ($detail_table_header_row + 2));
        $sheet->setCellValue("U" . ($detail_table_header_row + 1), '')->getStyle("U2")->applyFromArray($styleHeader)->getFont()->setSize(14)->setBold(true);
        $sheet->mergeCells("U" . ($detail_table_header_row + 1) . ":U" . ($detail_table_header_row + 2));
        ...
        foreach ($data as $response_index => $response_detail) {
            ...
            $sheet->setCellValue("U$rowIndex", $response_detail['nobuktikbtebs2']);
            ...
            $sheet->setCellValue("Y$rowIndex", $response_detail['totalbiaya'])->getStyle("Y$rowIndex")->applyFromArray($style_number)->getNumberFormat()->setFormatCode("#,##0.00_);(#,##0.00)");
            $sheet->setCellValue("Z$rowIndex", "=O$rowIndex-Y$rowIndex")->getStyle("Z$rowIndex")->applyFromArray($style_number)->getNumberFormat()->setFormatCode("#,##0.00_);(#,##0.00)");
            $sheet->setCellValue("AA$rowIndex", $response_detail['nobukti']);
            $rowIndex++;
