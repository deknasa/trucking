...
class LaporanUangJalanController extends MyController
{
    ...
    public function get($params = []): array
    {
        ...
        $response = Http::withHeaders(request()->header())
            ...
            ->get(config('app.api_url') . 'laporanuangjalan', $params);
 ...
 foreach ($pengeluaran as $response_index => $response_detail) {
            ...
            $sheet->setCellValue("B$detail_start_row", date('d-m-Y', strtotime($response_detail['tglabsensi'])));
            $sheet->setCellValue("C$detail_start_row", number_format($response_detail['nominalambil'], 2, ',', '.'));
            $sheet->setCellValue("D$detail_start_row", date('d-m-Y', strtotime($response_detail['tglkembali'])));
...
// $sheet->getStyle("A" . ($detail_start_row + 1) . ":$lastColumn" . ($detail_start_row + 1))->applyFromArray($border_style);
        ...
        $filename = 'EXPORTUANGJALAN' . date('dmYHis');
