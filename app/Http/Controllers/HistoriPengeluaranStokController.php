...
class HistoriPengeluaranStokController extends MyController
{
    public $title = 'Histori Pengeluaran Stok';

    public function index(Request $request)
    {
        $title = $this->title;
        $data = [
            'pagename' => 'Menu Utama Histori Pengeluaran Stok',
        ];

        return view('historipengeluaranstok.index', compact('title'));
    }
    /**
     * @ClassName
     */
    public function report(Request $request)
    {
        ...
        $header = Http::withHeaders(request()->header())
            ...
            ->get(config('app.api_url') . 'historipengeluaranstok/report', $detailParams);

        $data = $header['data'];
        $dataHeader = $header['dataheader'];
        $user = Auth::user();
        return view('reports.historipengeluaranstok', compact('data', 'user', 'dataHeader'));
    }
    /**
     * @ClassName
     */
    public function export(Request $request)
    {
        ...
        $historipengeluaranstok = $responses['data'];
        $user = Auth::user();
        ...
        $sheet->setCellValue('A2', 'Laporan Histori Pengeluaran Stok');
        ...
        foreach ($historipengeluaranstok as $response_index => $response_detail) {
            ...
            $sheet->setCellValue("F$detail_start_row", number_format((float) $response_detail['qtykeluar'], '2', '.', ','));
            $sheet->setCellValue("G$detail_start_row", number_format((float) $response_detail['nilaikeluar'], '2', '.', ','));
