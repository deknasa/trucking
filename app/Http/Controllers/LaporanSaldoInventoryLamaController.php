...
class LaporanSaldoInventoryLamaController extends MyController
{
    public $title = 'Laporan Saldo Inventory Lama';

    public function index(Request $request)
    {
        $title = $this->title;
        $data = [
            'pagename' => 'Menu Utama Laporan Saldo Inventory Lama',
        ];

        return view('laporansaldoinventorylama.index', compact('title'));
    }
    ...
    public function report(Request $request)
    {
        ...
        $header = Http::withHeaders(request()->header())
            ...
            ->get(config('app.api_url') . 'laporansaldoinventorylama/report', $detailParams);

        $data = $header['data'];
        $user = Auth::user();
        $opname['opname'] = $header['opname'];

        return view('reports.laporansaldoinventorylama', compact('data', 'user', 'detailParams', 'opname'));
    }
    ...
    public function export(Request $request): void
    {
        ...
        $sheet->setCellValue('A2', 'LAPORAN SALDO INVENTORY LAMA');
        ...
        $filename = 'LAPORAN SALDO INVENTORY LAMA' . date('dmYHis');
