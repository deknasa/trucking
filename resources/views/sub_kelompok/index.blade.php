...
<script>
  ...
  $(document).ready(function() {
    ...
    $("#jqGrid").jqGrid({
        url: `{{ config('app.api_url') . 'sub_kelompok' }}`,
        mtype: "GET",
        styleUI: 'Bootstrap4',
        iconSet: 'fontAwesome',
        datatype: "json",
      ...
      .navButtonAdd(pager, {
        ...
        onClickButton: function() {
          ...
          window.location.href = `{{ route('sub_kelompok.create') }}?sortname=${sortname}&sortorder=${sortorder}&limit=${limit}`
        }
      })
    ...
    if (!`{{ $myAuth->hasPermission('sub_kelompok', 'export') }}`) {
      $('#export').addClass('ui-disabled')
    }

    if (!`{{ $myAuth->hasPermission('sub_kelompok', 'report') }}`) {
    ...
    $('#formRange').submit(function(event) {
      ...
      if ($('#rangeModal').data('action') == 'export') {
        ...
        xhr.open('GET', `{{ config('app.api_url') }}sub_kelompok/export?${params}`, true)
        ...
        window.open(`{{ route('sub_kelompok.report') }}?${params}`)
