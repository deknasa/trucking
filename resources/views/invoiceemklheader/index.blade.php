...
@include('invoiceemklheader._modal')
<!-- Detail -->
@include('invoiceemklheader._detail')
@include('invoiceemklheader._piutang')
...
<script>
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '#btnReload', function(event) {
      loadDataHeader('invoiceemklheader')
      selectedRows = []
      selectedbukti = []
      $('#gs_').prop('checked', false)
    })
    ...
    grid.jqGrid({
        ...
        colModel: [{
          ...
          {
            label: 'NO BUKTI INVOICE PAJAK',
            name: 'nobuktiinvoicepajak',
            width: (detectDeviceType() == "desktop") ? sm_dekstop_4 : sm_mobile_3,
            align: 'left'
          },
          {
            label: 'NO BUKTI INVOICE REIMBURSMENT',
            name: 'nobuktiinvoicereimbursement',
            width: (detectDeviceType() == "desktop") ? sm_dekstop_4 : sm_mobile_3,
            align: 'left'
          },
          {
            label: 'NO BUKTI INVOICE NONPAJAK',
            name: 'nobuktiinvoicenonpajak',
            width: (detectDeviceType() == "desktop") ? sm_dekstop_4 : sm_mobile_3,
            align: 'left'
          },
          {
          ...
          },
          {
            ...
            formatter: (value, options, rowData) => {
              ...
              let tgldari = rowData.tgldariheaderpiutang
              let tglsampai = rowData.tglsampaiheaderpiutang
        ...
        modalBtnList: [
          ...
          },
          {
            ...
            onClick: () => {
              ...
              if (selectedId == null || selectedId == '' || selectedId == undefined) {
                ...
                $.ajax({
                  url: `${apiUrl}invoiceemklheader/${selectedId}/export`,
                  type: 'GET',
                  data: {
                    forReport : true,
                    invoiceemkl_id: selectedId,
                    export : true
                  },
                  ...
                  success: function(response, status, xhr) {
                    if (xhr.status === 200) {
                      if (response !== undefined) {
                        ...
                        link.download = 'LAPORAN INVOICE EMKL' + new Date().getTime() + '.xlsx';
                        link.click();
                      }
                    }
          ...
          },
          {
            ...
            item: [{
                ...
                hidden: (!`{{ $myAuth->hasPermission('invoiceemklheader', 'approval') }}`),
                onClick: () => {

                  if (`{{ $myAuth->hasPermission('invoiceemklheader', 'approval') }}`) {
                    approve()
                  }
                }
              },
              {
                ...
                hidden: (!`{{ $myAuth->hasPermission('invoiceemklheader', 'approvalbukacetak') }}`),
                onClick: () => {
                  if (`{{ $myAuth->hasPermission('invoiceemklheader', 'approvalbukacetak') }}`) {
                    let tglbukacetak = $('#tgldariheader').val().split('-');
                    tglbukacetak = tglbukacetak[1] + '-' + tglbukacetak[2];
                    if (selectedRows.length < 1) {
                      showDialog('Harap pilih salah satu record')
                    } else {
                      approvalBukaCetak(tglbukacetak, 'INVOICEEMKLHEADER', selectedRows, selectedbukti);
                    }
                  }
                }
              },
    ...
    function permission() {
      if (!`{{ $myAuth->hasPermission('invoiceemklheader', 'store') }}`) {
        $('#add').attr('disabled', 'disabled')
      }
      if (!`{{ $myAuth->hasPermission('invoiceemklheader', 'show') }}`) {
        $('#view').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('invoiceemklheader', 'update') }}`) {
        $('#edit').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('invoiceemklheader', 'destroy') }}`) {
        $('#delete').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('invoiceemklheader', 'export') }}`) {
        $('#export').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('invoiceemklheader', 'report') }}`) {
