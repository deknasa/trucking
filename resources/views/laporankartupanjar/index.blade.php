...
<script>
    ...
    $(document).ready(function() {
        ...
        if (!`{{ $myAuth->hasPermission('laporankartupanjar', 'report') }}`) {
            $('#btnPreview').attr('disabled', 'disabled')
        }
        if (!`{{ $myAuth->hasPermission('laporankartupanjar', 'export') }}`) {
    ...
    $(document).on('click', `#btnExport`, function(event) {
        ...
        $.ajax({
            ...
            // url: `{{ route('laporankartupanjar.export') }}?dari=${dari}&sampai=${sampai}&agendari_id=${agendari_id}&agensampai_id=${agensampai_id}&agendari=${agendari}&agensampai=${agensampai}`,
            type: 'GET',
            data: {
                dari: dari,
                sampai: sampai,
    ...
    function laporankartupanjar(data, detailParams, dataCabang,cabang) {
        ...
        if (cabang == 'MEDAN') {
            report.loadFile(`{{ asset('public/reports/ReportLaporanKartuPanjarA4.mrt') }}`);
        }else if(cabang == 'MAKASSAR'){
            report.loadFile(`{{ asset('public/reports/ReportLaporanKartuPanjarLetter.mrt') }}`);
        }else{
            report.loadFile(`{{ asset('public/reports/ReportLaporanKartuPanjar.mrt') }}`);
        }
