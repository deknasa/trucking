                ...
                $('#formRange').submit(event => {
                    ...
                    getCekExport(params).then((response) => {
                        if ($('#rangeModal').data('action') == 'export') {
                            $.ajax({
                                url: '{{ config('app.api_url') }}kelompok/export?' + params,
                                type: 'GET',
                                    ...
                                    if (xhr.status === 200) {
                                        if (response !== undefined) {
                                            var blob = new Blob([response], {
                                                type: 'kelompok/vnd.ms-excel'
                                            });
                                            var link = document.createElement('a');
                                            link.href = window.URL.createObjectURL(blob);
                                            link.download = 'kelompok' + new Date().getTime() + '.xlsx';
                                            link.click();
                                        }
                            ...
                            window.open(`{{ route('kelompok.report') }}?${params}`)
                            submitButton.removeAttr('disabled')
                            $('#processingLoader').addClass('d-none')
                            $('#rangeModal').modal('hide')
                        }    
          ...
          $('#processingLoader').addClass('d-none')
                ...
                function getCekExport(params) {
                    ...
                    return new Promise((resolve, reject) => {
                        $.ajax({
                            url: `${apiUrl}kelompok/export?${params}`,
                            dataType: "JSON",
                            headers: {
