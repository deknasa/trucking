          ...
          <div class="card-body">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="id" class="form-control" value="{{ $kelompok['id'] ?? '' }}" readonly>
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="kodekelompok" class="form-control" value="{{ $kelompok['kodekelompok'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="keterangan" class="form-control" value="{{ $kelompok['keterangan'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
              ...
              <div class="col-12 col-md-10">
                <select name="statusaktif" class="form-control select2bs4">
                        ...
                        <?php foreach ($combo['statusaktif'] as $key => $item) { 
                            $selected = @$kelompok['statusaktif'] == $item['id'] ? "selected" : ""
                        ?>
...
@push('scripts')
<script>
  let indexUrl = "{{ route('kelompok.index') }}"
  let fieldLengthUrl = "{{ route('kelompok.field_length') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('kelompok.store') }}"
  let method = "POST"
  let csrfToken = "{{ csrf_token() }}"

  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('kelompok.update', $kelompok['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('kelompok.destroy', $kelompok['id']) }}"
    method = "DELETE"
  <?php endif; ?>
