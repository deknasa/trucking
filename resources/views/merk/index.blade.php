...
<script>
    ...
    function selectAllRows() {
        $.ajax({
            ...
            success: (response) => {
                selectedRows = response.data.map((merk) => merk.id)
                $('#jqGrid').trigger('reloadGrid')
            }
        })
    }
    ...
    $(document).ready(function() {
            ...
            .customPager({
                ...
                modalBtnList: [{
                    ...
                    item: [{
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('merk', 'approvalaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('merk', 'approvalaktif') }}`) {
                                    approvalAktif('merk')
                        ...
                        },
                        {
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('merk', 'approvalnonaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('merk', 'approvalnonaktif') }}`) {
                                    approvalNonAktif('merk')
                                }
                            }
                        },
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('merk', 'report') }}`) {
        ...
        $('#formRange').submit(event => {
            ...
            getCekExport(params).then((response) => {
                    if ($('#rangeModal').data('action') == 'export') {
                                ...
                                if (xhr.status === 200) {
                                    if (response !== undefined) {
                                        var blob = new Blob([response], {
                                            type: 'merk/vnd.ms-excel'
                                        });
                                        var link = document.createElement('a');
                                        link.href = window.URL.createObjectURL(blob);
                                        link.download = 'merk' + new Date().getTime() + '.xlsx';
                                        link.click();
                                    }
                        ...
                        window.open(`{{ route('merk.report') }}?${params}`)
                        submitButton.removeAttr('disabled')
                        $('#processingLoader').addClass('d-none')
                        $('#rangeModal').modal('hide')
                    }
