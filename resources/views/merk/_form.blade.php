      ...
      <div class="card card-primary">
        ...
        <form action="" method="post">
          <div class="card-body">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="id" class="form-control" value="{{ $merk['id'] ?? '' }}" readonly>
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
                <label>
                  KODE MERK <span class="text-danger">*</span>
                </label>
              </div>
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="kodemerk" class="form-control" value="{{ $merk['kodemerk'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
              ...
              <div class="col-12 col-md-10">
                <select name="statusaktif" class="form-control select2bs4">
                        ...
                        <?php foreach ($combo['statusaktif'] as $key => $item) { 
                            $selected = @$merk['statusaktif'] == $item['id'] ? "selected" : ""
                        ?>
          ...
          <div class="card-footer">
            ...
            <a href="{{ route('merk.index') }}" class="btn btn-danger">
...
@push('scripts')
<script>
  let indexUrl = "{{ route('merk.index') }}"
  let fieldLengthUrl = "{{ route('merk.field_length') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('merk.store') }}"
  let method = "POST"
  let csrfToken = "{{ csrf_token() }}"

  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('merk.update', $merk['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('merk.destroy', $merk['id']) }}"
    method = "DELETE"
  <?php endif; ?>
