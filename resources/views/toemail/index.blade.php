...
<script>
  ...
  function selectAllRows() {
    $.ajax({
      ...
      success: (response) => {
        selectedRows = response.data.map((toemail) => toemail.id)
        $('#jqGrid').trigger('reloadGrid')
      }
    })
  }
  ...
  $(document).ready(function() {
    ...
    function permission() {
      ...
      if (!`{{ $myAuth->hasPermission('toemail', 'report') }}`) {
