        ...
        <form action="" method="post">
          <div class="card-body">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="kodekategori" class="form-control" value="{{ $kategori['kodekategori'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-md-10">
                <select name="subkelompok_id" class="form-control select2bs4">
                        <option value="">PILIH STATUS</option>
                        <?php foreach ($combo['subkelompok'] as $key => $item) { 
                            $selected = @$kategori['subkelompok_id'] == $item['id'] ? "selected" : ""
                        ?>
              ...
              <div class="col-12 col-md-10">
                <select name="statusaktif" class="form-control select2bs4">
                        ...
                        <?php foreach ($combo['statusaktif'] as $key => $item) { 
                            $selected = @$kategori['statusaktif'] == $item['id'] ? "selected" : ""
                        ?>
...
@push('scripts')
<script>
  let indexUrl = "{{ route('kategori.index') }}"
  let fieldLengthUrl = "{{ route('kategori.field_length') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('kategori.store') }}"
  let method = "POST"
  let csrfToken = "{{ csrf_token() }}"

  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('kategori.update', $kategori['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('kategori.destroy', $kategori['id']) }}"
    method = "DELETE"
  <?php endif; ?>
