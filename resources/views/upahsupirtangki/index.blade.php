...
<!-- Detail -->
@include('upahsupirtangki._detail')

@include('upahsupirtangki._modal')
...
<script>
  ...
  $(document).ready(function() {
            ...
            formatter: (value, row) => {
              ...
              if (value) {
                ...
                files.forEach(file => {
                  ...
                  image.height = 25
                  image.src =
                    `${apiUrl}upahsupirtangki/${encodeURI(file)}/small`
                ...
                image.width = 25
                image.height = 25
                image.src = `${apiUrl}upahsupirtangki/no-image/small`
      ...
      .customPager({
        ...
        modalBtnList: [{
          ...
          item: [{
              ...
              hidden: (!`{{ $myAuth->hasPermission('upahsupirtangki', 'approvalaktif') }}`),
              onClick: () => {
                if (`{{ $myAuth->hasPermission('upahsupirtangki', 'approvalaktif') }}`) {
                  approvalAktif('upahsupirtangki')
            ...
            },
            {
              ...
              hidden: (!`{{ $myAuth->hasPermission('upahsupirtangki', 'approvalnonaktif') }}`),
              onClick: () => {
                if (`{{ $myAuth->hasPermission('upahsupirtangki', 'approvalnonaktif') }}`) {
                  approvalNonAktif('upahsupirtangki')
                }
              }
            },
          ],
    ...
    function permission() {
      if (!`{{ $myAuth->hasPermission('upahsupirtangki', 'store') }}`) {
        $('#add').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('upahsupirtangki', 'update') }}`) {
        $('#edit').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('upahsupirtangki', 'destroy') }}`) {
        $('#delete').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('upahsupirtangki', 'export') }}`) {
        $('#export').attr('disabled', 'disabled')
      }

      if (!`{{ $myAuth->hasPermission('upahsupirtangki', 'report') }}`) {
      ...
      getCekExport()
        .then((response) => {
          if ($('#formRangeTgl').data('action') == 'export') {
            actionUrl = `{{ route('upahsupirtangki.export') }}`
          } else {
            actionUrl = `{{ route('upahsupirtangki.report') }}`
          }
    ...
    function getCekExport() {
      return new Promise((resolve, reject) => {
        $.ajax({
          url: `${apiUrl}upahsupirtangki/export`,
          dataType: "JSON",
          headers: {
            Authorization: `Bearer ${accessToken}`
          },
