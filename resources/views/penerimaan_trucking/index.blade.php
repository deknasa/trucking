...
@push('scripts')
<script>
  let indexUrl = "{{ route('penerimaan_trucking.index') }}"
  let indexRow = 0;
  let page = 1;
  ...
  $(document).ready(function() {
    ...
    $("#jqGrid").jqGrid({
        url: `{{ config('app.api_url') . 'penerimaan_trucking' }}`,
        mtype: "GET",
        styleUI: 'Bootstrap4',
        iconSet: 'fontAwesome',
        datatype: "json",
      ...
      .navButtonAdd(pager, {
        ...
        onClickButton: function() {
          ...
          window.location.href = `{{ route('penerimaan_trucking.create') }}?sortname=${sortname}&sortorder=${sortorder}&limit=${limit}`
        }
      })
    ...
    if (!`{{ $myAuth->hasPermission('penerimaan_trucking', 'export') }}`) {
      $('#export').addClass('ui-disabled')
    }

    if (!`{{ $myAuth->hasPermission('penerimaan_trucking', 'report') }}`) {
    ...
    $('#formRange').submit(function(event) {
      ...
      if ($('#rangeModal').data('action') == 'export') {
        ...
        xhr.open('GET', `{{ config('app.api_url') }}penerimaan_trucking/export?${params}`, true)
        ...
        window.open(`{{ route('penerimaan_trucking.report') }}?${params}`)
