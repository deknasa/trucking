...
<head>
  ...
  <title>Report Invoice Charge Gandengan</title>
  ...
  <script type="text/javascript">
    let chargegandengan = <?= json_encode($chargegandengan); ?>;
    ...
    function Start() {
      ...
      var statuscetak = chargegandengan.statuscetak_id
      var sudahcetak = chargegandengan['combo']['id']
     ...
     //  options.appearance.fullScreenMode = true
      ...
      if (printer['tipe'] == 'reportPrinterBesar') {
        report.loadFile(`{{ asset('public/reports/ReportInvoiceChargeGandenganBesar.mrt') }}`)
      } else {
        report.loadFile(`{{ asset('public/reports/ReportInvoiceChargeGandengan.mrt') }}`)
      }

      report.dictionary.dataSources.clear()

      dataSet.readJson({
        'chargegandengan': <?= json_encode($chargegandengan); ?>,
        'chargegandengan_details': <?= json_encode($chargegandengan_details); ?>
      })
      ...
      window.addEventListener('afterprint', (event) => {
        var id = chargegandengan.id
        var apiUrl = `{{ config('app.api_url') }}`;
        $.ajax({
          url: `${apiUrl}invoicechargegandenganheader/${id}/printreport`,
          method: 'GET',
          dataType: 'JSON',
          headers: {
