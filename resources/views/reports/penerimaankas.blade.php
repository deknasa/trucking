...
<head>
  ...
  <script type="text/javascript">
    ...
    function Start() {
      ...
      var statuscetak = penerimaanheader.statuscetak_id
      var sudahcetak = penerimaanheader['combo']['id']
      ...
      if (cabang['cabang'] == 'PUSAT') {
        if (printer['tipe'] == 'reportPrinterBesar') {
          report.loadFile(`{{ asset('public/reports/ReportPenerimaanKasBesarPusat.mrt') }}`)
        } else {
          report.loadFile(`{{ asset('public/reports/ReportPenerimaanKasPusat.mrt') }}`)
        }

      } else {
        if (printer['tipe'] == 'reportPrinterBesar') {
          report.loadFile(`{{ asset('public/reports/ReportPenerimaanKasBesar.mrt') }}`)
        } else {
          report.loadFile(`{{ asset('public/reports/ReportPenerimaanKas.mrt') }}`)
        }
      ...
      dataSet.readJson({
        ...
        'penerimaan_details': <?= json_encode($penerimaan_details); ?>
      })
