...
<head>
    ...
    <script type="text/javascript">
        let hutangheader = <?= json_encode($hutang) ?>;
        ...
        function Start() {
            ...
            Stimulsoft.Report.Dictionary.StiFunctions.addFunction("MyCategory", "Terbilang", "Terbilang", "Terbilang", "",
                String, "Return Description", [Object], ["value"], ["Descriptions"], terbilang);
            viewerOptions.toolbar.viewMode = Stimulsoft.Viewer.StiWebViewMode.Continuous;

            var viewer = new Stimulsoft.Viewer.StiViewer(viewerOptions, "StiViewer", false)
            var report = new Stimulsoft.Report.StiReport()

            var statuscetak = hutangheader.statuscetak_id
            var sudahcetak = hutangheader['combo']['id']
            ...
            if (printer['tipe'] == 'reportPrinterBesar') {
                report.loadFile(`{{ asset('public/reports/ReportHutangBesar.mrt') }}`)
            } else {
                report.loadFile(`{{ asset('public/reports/ReportHutang.mrt') }}`)
            }

            report.dictionary.dataSources.clear()

            dataSet.readJson({
                'hutang': <?= json_encode($hutang) ?>,
                'hutang_details': <?= json_encode($hutang_details) ?>
            })
            ...
            window.addEventListener('afterprint', (event) => {
                ...
                $.ajax({
                    url: `${apiUrl}hutangheader/${id}/printreport`,
                    method: 'GET',
                    dataType: 'JSON',
                    headers: {
        ...
        $(document).ready(function() {
            ...
            if (statuscetak == sudahcetak) {
                $(document).on('keydown', function(e) {
                    if ((e.ctrlKey || e.metaKey) && (e.key == "p" || e.charCode == 16 || e.charCode ==
                            112 || e.keyCode == 80)) {
