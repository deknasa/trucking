...
<head>
  ...
  <title>Report Hutang Bayar</title>
  ...
  <script type="text/javascript">
    ...
    let hutangbayarheaders = <?= json_encode($hutangbayar); ?>

    console.log(hutangbayarheaders.statuscetak)
    $( document ).ready(function() {
      var statuscetak = hutangbayarheaders.statuscetak
      if (statuscetak == 174) {
    ...
    function Start() {
      ...
      report.loadFile(`{{ asset('public/reports/ReportHutangBayar.mrt') }}`)

      report.dictionary.dataSources.clear()

      dataSet.readJson({
        'hutangbayar_details': <?= json_encode($hutangbayar_details); ?>,
        'hutangbayar': <?= json_encode($hutangbayar); ?>
      })
    ...
    //   designer.renderHtml('content');
      ...
      window.addEventListener('afterprint', (event) => {
        ...
        var id = hutangbayarheaders.id
        var apiUrl = `{{ config('app.api_url') }}`;
        
        $.ajax({
          url: `${apiUrl}hutangbayarheader/${id}/printreport`,
          method: 'GET',
          dataType: 'JSON',
          headers: {
