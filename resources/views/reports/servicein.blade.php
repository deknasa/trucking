...
<head>
  ...
  <script type="text/javascript">
    let serviceInheader = <?= json_encode($serviceIn); ?>;
    ...
    function Start() {
      ...
      var statuscetak = serviceInheader.statuscetak_id
      var sudahcetak = serviceInheader['combo']['id']
      ...
      if (printer['tipe'] == 'reportPrinterBesar') {
        report.loadFile(`{{ asset('public/reports/ReportServiceInBesar.mrt') }}`)
      } else {
        report.loadFile(`{{ asset('public/reports/ReportServiceIn.mrt') }}`)
      }

      report.dictionary.dataSources.clear()

      dataSet.readJson({
        'serviceIn': <?= json_encode($serviceIn); ?>,
        'serviceIn_details': <?= json_encode($serviceIn_details); ?>
      })
      ...
      window.addEventListener('afterprint', (event) => {
        ...
        $.ajax({
          url: `${apiUrl}serviceinheader/${id}/printreport`,
          method: 'GET',
          dataType: 'JSON',
          headers: {
