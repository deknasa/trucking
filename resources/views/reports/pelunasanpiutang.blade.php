...
<head>
  ...
  <script type="text/javascript">
    let pelunasanPiutangs = <?= json_encode($pelunasanPiutangs); ?>;
    ...
    function Start() {
      ...
      if (printer['tipe'] == 'reportPrinterBesar') {
        report.loadFile(`{{ asset('public/reports/ReportPelunasanPiutangBesar.mrt') }}`)
      } else {
        report.loadFile(`{{ asset('public/reports/ReportPelunasanPiutang.mrt') }}`)
      }

      report.dictionary.dataSources.clear()

      dataSet.readJson({
        'pelunasanPiutangs': <?= json_encode($pelunasanPiutangs); ?>,
        'pelunasanPiutang_details': <?= json_encode($pelunasanPiutang_details); ?>
      })
      ...
      window.addEventListener('afterprint', (event) => {
        ...
        $.ajax({
          url: `${apiUrl}pelunasanpiutangheader/${id}/printreport`,
          method: 'GET',
          dataType: 'JSON',
          headers: {
