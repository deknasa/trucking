...
<head>
  ...
  <script type="text/javascript">
    ...
    function Start() {
      ...
      var statuscetak = jurnal.statuscetak_id
      var sudahcetak = jurnal['combo']['id']
      ...
      if (printer['tipe'] == 'reportPrinterBesar') {
        report.loadFile(`{{ asset('public/reports/ReportJurnalUmumBesar.mrt') }}`)
      } else {
        report.loadFile(`{{ asset('public/reports/ReportJurnalUmum.mrt') }}`)
      }

      report.dictionary.dataSources.clear()

      dataSet.readJson({
        'jurnal': <?= json_encode($jurnal); ?>,
        'jurnal_details': <?= json_encode($jurnal_details); ?>,
      })
      ...
      window.addEventListener('afterprint', (event) => {
        ...
        $.ajax({
          url: `${apiUrl}jurnalumumheader/${id}/printreport`,
          method: 'GET',
          dataType: 'JSON',
          headers: {
