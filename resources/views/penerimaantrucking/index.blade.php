...
<script>
    ...
    $(document).ready(function() {
            ...
            .customPager({
                buttons: [{
                    ...
                    },
                    {
                        ...
                        onClick: () => {
                            selectedId = $("#jqGrid").jqGrid('getGridParam', 'selrow')
                            viewPenerimaanTrucking(selectedId)
                        }
                    },
                    {
                        id: 'report',
                ...
                modalBtnList: [{
                    ...
                    item: [{
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('penerimaantrucking', 'approvalaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('penerimaantrucking', 'approvalaktif') }}`) {
                                    approvalAktif('penerimaantrucking')
                        ...
                        },
                        {
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('penerimaantrucking', 'approvalnonaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('penerimaantrucking', 'approvalnonaktif') }}`) {
                                    approvalNonAktif('penerimaantrucking')
                                }
                            }
                        },
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('penerimaantrucking', 'show') }}`) {
                                ...
                                if (xhr.status === 200) {
                                    if (response !== undefined) {
                                        var blob = new Blob([response], {
                                            type: 'penerimaantrucking/vnd.ms-excel'
                                        });
                                        var link = document.createElement('a');
                                        link.href = window.URL.createObjectURL(blob);
                                        link.download = 'penerimaantrucking' + new Date().getTime() + '.xlsx';
                                        link.click();
                                    }
