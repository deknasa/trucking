...
<script>
    ...
    let limit
    let postData
    let sortname = 'kodegandengan'
    let sortorder = 'asc'
    let autoNumericElements = []
    let indexRow = 0;
    let selectedRows = [];
    let selectedRowsGandengan = [];
    ...
    function checkboxHandler(element) {
        ...
        if (element.checked) {
            ...
            selectedRowsGandengan.push($(element).parents('tr').find(`td[aria-describedby="jqGrid_kodegandengan"]`).text())
            ...
            for (var i = 0; i < selectedRows.length; i++) {
                if (selectedRows[i] == value) {
                    ...
                    selectedRowsGandengan.splice(i, 1);
                }
            }
        }
    ...
    $(document).ready(function() {
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('gandengan', 'export') }}`) {
        ...
        $('#formRange').submit(function(event) {
            ...
            getCekExport(params).then((response) => {
                    if ($('#rangeModal').data('action') == 'export') {
                                ...
                                if (xhr.status === 200) {
                                    if (response !== undefined) {
                                        var blob = new Blob([response], {
                                            type: 'gandengan/vnd.ms-excel'
                                        });
                                        var link = document.createElement('a');
                                        link.href = window.URL.createObjectURL(blob);
                                        link.download = 'laporangandengan' + new Date().getTime() + '.xlsx';
                                        link.click();
                                    }
                        ...
                        window.open(`{{ route('gandengan.report') }}?${params}`)
                        submitButton.removeAttr('disabled')
                        $('#processingLoader').addClass('d-none')
                        $('#rangeModal').modal('hide')
                    }
