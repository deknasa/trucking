...
<script>
  ...
  $(document).ready(function() {
    ...
    function submit(button) {
      ...
      switch (action) {
        ...
        case 'delete':
          method = 'DELETE'
          url = `${apiUrl}notakreditheader/${Id}?tgldariheader=${tgldariheader}&tglsampaiheader=${tglsampaiheader}&indexRow=${indexRow}&limit=${limit}&page=${page}`
          break;
        default:
          method = 'POST'
          url = `${apiUrl}notakreditheader`
  ...
  function removeEditingBy(id) {
    ...
    formData.append('table', 'notakreditheader');
  ...
  `)
    ...
    $('#crudModalTitle').text('Add Nota Kredit')
    $('.is-invalid').removeClass('is-invalid')
    $('.invalid-feedback').remove()
  ...
  function viewNotaKreditHeader(userId) {
    let form = $('#crudForm')
    $('.modal-loader').removeClass('d-none')
    ...
    `)
    form.find(`.sometimes`).hide()
    $('#crudModalTitle').text('View Nota Kredit')
    $('.is-invalid').removeClass('is-invalid')
    $('.invalid-feedback').remove()
  ...
  function cekValidasiAksi(Id, Aksi) {
    $.ajax({
      url: `{{ config('app.api_url') }}notakreditheader/${Id}/cekValidasiAksi`,
      method: 'POST',
      dataType: 'JSON',
      beforeSend: request => {
        request.setRequestHeader('Authorization', `Bearer {{ session('access_token') }}`)
      ...
      $.ajax({
        ...
        success: response => {
          ...
          $.each(response.detail, (index, detail) => {
            ...
            detailRow.find(`[name="nominal_detail[]"]`).val(detail.penyesuaian)
  ...
  function showDefault(form) {
    return new Promise((resolve, reject) => {
      $.ajax({
        url: `${apiUrl}notakreditheader/default`,
        method: 'GET',
        dataType: 'JSON',
        headers: {
          Authorization: `Bearer ${accessToken}`
