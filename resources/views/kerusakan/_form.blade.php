      ...
      <div class="card card-primary">
        ...
        <form action="" method="post">
          <div class="card-body">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="id" class="form-control" value="{{ $kerusakan['id'] ?? '' }}" readonly>
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="keterangan" class="form-control" value="{{ $kerusakan['keterangan'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
          ...
          <div class="card-footer">
            ...
            <a href="{{ route('kerusakan.index') }}" class="btn btn-danger">
...
@push('scripts')
<script>
  let indexUrl = "{{ route('kerusakan.index') }}"
  let fieldLengthUrl = "{{ route('kerusakan.field_length') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('kerusakan.store') }}"
  let method = "POST"
  let csrfToken = "{{ csrf_token() }}"

  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('kerusakan.update', $kerusakan['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('kerusakan.destroy', $kerusakan['id']) }}"
    method = "DELETE"
  <?php endif; ?>
