...
<script>
    ...
    function selectAllRows() {
        $.ajax({
            ...
            success: (response) => {
                selectedRows = response.data.map((container) => container.id)
                $('#jqGrid').trigger('reloadGrid')
            }
        })
    }
            ...
            .customPager({
                ...
                modalBtnList: [{
                    ...
                    item: [{
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('container', 'approvalaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('container', 'approvalaktif') }}`) {
                                    approvalAktif('container')
                        ...
                        },
                        {
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('container', 'approvalnonaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('container', 'approvalnonaktif') }}`) {
                                    approvalNonAktif('container')
                                }
                            }
                        },
        ...
        $('#formRange').submit(event => {
            ...
            getCekExport(params).then((response) => {
                if ($('#rangeModal').data('action') == 'export') {
                    $.ajax({
                        url: `${apiUrl}container/export?${params}`,
                        type: 'GET',
                            ...
                            if (xhr.status === 200) {
                                if (response !== undefined) {
                                    var blob = new Blob([response], {
                                        type: 'container/vnd.ms-excel'
                                    });
                                    var link = document.createElement('a');
                                    link.href = window.URL.createObjectURL(blob);
                                    link.download = 'laporancontainer' + new Date().getTime() + '.xlsx';
                                    link.click();
                                }
                    ...
                    window.open(`{{ route('container.report') }}?${params}`)
                    submitButton.removeAttr('disabled')
                    $('#processingLoader').addClass('d-none')
                    $('#rangeModal').modal('hide')
                }
