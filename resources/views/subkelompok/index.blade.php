...
<script>
    ...
    function selectAllRows() {
        $.ajax({
            ...
            success: (response) => {
                selectedRows = response.data.map((subkelompok) => subkelompok.id)
                $('#jqGrid').trigger('reloadGrid')
            }
        })
    }
    ...
    $(document).ready(function() {
            ...
            .customPager({
                ...
                modalBtnList: [{
                    ...
                    item: [{
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('subkelompok', 'approvalaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('subkelompok', 'approvalaktif') }}`) {
                                    approvalAktif('subkelompok')
                        ...
                        },
                        {
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('subkelompok', 'approvalnonaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('subkelompok', 'approvalnonaktif') }}`) {
                                    approvalNonAktif('subkelompok')
                                }
                            }
                        },
        ...
        function permission() {
            if (cabangTnl == 'YA') {
                ...
                if (!`{{ $myAuth->hasPermission('subkelompok', 'destroy') }}`) {
                    $('#delete').attr('disabled', 'disabled')
                }
            }
            ...
            if (!`{{ $myAuth->hasPermission('subkelompok', 'export') }}`) {
                $('#export').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('subkelompok', 'report') }}`) {
        ...
        $('#formRange').submit(function(event) {
            ...
            getCekExport(params).then((response) => {
                    if ($('#rangeModal').data('action') == 'export') {
                        $.ajax({
                            url: '{{ config('
                            app.api_url ') }}subkelompok/export?' + params,
                            type: 'GET',
                                ...
                                if (xhr.status === 200) {
                                    if (response !== undefined) {
                                        var blob = new Blob([response], {
                                            type: 'subkelompok/vnd.ms-excel'
                                        });
                                        var link = document.createElement('a');
                                        link.href = window.URL.createObjectURL(blob);
                                        link.download = 'subkelompok' + new Date().getTime() + '.xlsx';
                                        link.click();
                                    }
                        ...
                        window.open(`{{ route('subkelompok.report') }}?${params}`)
                        submitButton.prop('disabled', false)
                        $('#processingLoader').addClass('d-none')
                        $('#rangeModal').modal('hide')
                    }
        ...
        function getCekExport(params) {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: `${apiUrl}subkelompok/export?${params}`,
                    dataType: "JSON",
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    },
