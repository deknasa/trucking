...
<script>
    ...
    $(document).ready(function() {
                ...
                buttons: [{
                    ...
                    onClick: function(event) {
                        ...
                        if (dari != '' && sampai != '' && filter != '') {
                            ...
                            window.open(
                                `{{ route('kartustoklama.report') }}?dari=${dari}&sampai=${sampai}&stokdari_id=${stokdari_id}&stoksampai_id=${stoksampai_id}&kelompok_id=${kelompok_id}&statustampil=${statustampil}&filter=${filter}&datafilter=${dataFilter}&proses=${proses}`
                            )
                        } else {
                            showDialog('ISI SELURUH KOLOM')
                        }
                    ...
                    onClick: function(event) {
                        ...
                        if (dari != '' && sampai != '' && filter != '') {
                            ...
                            // window.open(
                            //     `{{ route('kartustoklama.export') }}?dari=${dari}&sampai=${sampai}&stokdari_id=${stokdari_id}&stoksampai_id=${stoksampai_id}&kelompok_id=${kelompok_id}&statustampil=${statustampil}&filter=${filter}&datafilter=${dataFilter}&proses=${proses}`
                            // )
                            $.ajax({
                                url: `${apiUrl}kartustoklama/export`,
                                type: 'GET',
                                data: {
                                    stokdari_id : stokdari_id,
                                    stoksampai_id : stoksampai_id,
                                ...
                                success: function(response, status, xhr) {
                                    if (xhr.status === 200) {
                                        if (response !== undefined) {
                                            ...
                                            link.download = 'KARTU STOK LAMA' + new Date().getTime() + '.xlsx';
                                            link.click();
                                        }
                                    }
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('kartustoklama', 'report') }}`) {
                $('#report').attr('disabled', 'disabled')
            }
            if (!`{{ $myAuth->hasPermission('kartustoklama', 'export') }}`) {
    ...
    function showDefault(form) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: `${apiUrl}kartustoklama/default`,
                method: 'GET',
                dataType: 'JSON',
                headers: {
