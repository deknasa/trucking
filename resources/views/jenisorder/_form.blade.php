              ...
              <div class="col-12 col-md-10">
                <select name="statusaktif" class="form-control select2bs4">
                        ...
                        <?php foreach ($combo['statusaktif'] as $key => $item) { 
                            $selected = @$jenisorder['statusaktif'] == $item['id'] ? "selected" : ""
                        ?>
...
<script>
  ...
  let fieldLengthUrl = "{{ route('jenisorder.field_length') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('jenisorder.store') }}"
  let method = "POST"
  let csrfToken = "{{ csrf_token() }}"

  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('jenisorder.update', $jenisorder['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('jenisorder.destroy', $jenisorder['id']) }}"
    method = "DELETE"
  <?php endif; ?>
