...
<script>
    ...
    $(document).ready(function() {
        $("#jqGrid").jqGrid({
                ...
                colModel: [{
                        ...
                        formatter: (value, rowOptions, rowData) => {
                            return `<input type="checkbox" name="serapId[]" class="checkbox-jqgrid" value="${rowData.id}" onchange="checkboxHandler(this)">`
                        },
                    },
                    {
                        label: 'ID',
        ...
        function permission() {
            if (!`{{ $myAuth->hasPermission('supirserap', 'store') }}`) {
                $('#add').attr('disabled', 'disabled')
            }
            if (!`{{ $myAuth->hasPermission('supirserap', 'show') }}`) {
                $('#view').attr('disabled', 'disabled')
            }
            if (!`{{ $myAuth->hasPermission('supirserap', 'update') }}`) {
                $('#edit').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('supirserap', 'destroy') }}`) {
                $('#delete').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('supirserap', 'export') }}`) {
                $('#export').attr('disabled', 'disabled')
            }

            if (!`{{ $myAuth->hasPermission('supirserap', 'report') }}`) {
                $('#report').attr('disabled', 'disabled')
            }
            if (!`{{ $myAuth->hasPermission('supirserap', 'approval') }}`) {
        ...
        $('#formRangeTgl').submit(function(event) {
            ...
            if ($('#rangeTglModal').data('action') == 'report') {
                window.open(`{{ route('supirserap.report') }}?dari=${dari}&sampai=${sampai}`)
            } else {

                $('#processingLoader').removeClass('d-none')
                $.ajax({
                    url: `{{ route('supirserap.export') }}?${$('#formRangeTgl').serialize()}`,
                    type: 'GET',
