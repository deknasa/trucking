        ...
        <script>
            ...
            function selectAllRows() {
                $.ajax({
                    ...
                    success: (response) => {
                        selectedRows = response.data.map((pengeluaranstok) => pengeluaranstok.id)
                        $('#jqGrid').trigger('reloadGrid')
                    }
                })
            }
            ...
            `,
              ...
              `)
                    ...
                    .customPager({
                        buttons: [{
                            ...
                            },
                            {
                                ...
                                onClick: () => {
                                    selectedId = $("#jqGrid").jqGrid('getGridParam', 'selrow')
                                    viewPengeluaranStok(selectedId)
                                }
                            },  
                            {
                                id: 'report',
                ...
                function permission() {
                    ...
                    if (!`{{ $myAuth->hasPermission('pengeluaranstok', 'show') }}`) {
                ...
                $('#formRange').submit(function(event) {
                    ...
                    getCekExport(params).then((response) => {
                        if ($('#rangeModal').data('action') == 'export') {
                            $.ajax({
                                url: '{{ config('app.api_url') }}pengeluaranstok/export?' + params,
                                type: 'GET',
                                  ...
                                  if (xhr.status === 200) {
                                    if (response !== undefined) {
                                      var blob = new Blob([response], {
                                        type: 'pengeluaranstok/vnd.ms-excel'
                                      });
                                      var link = document.createElement('a');
                                      link.href = window.URL.createObjectURL(blob);
                                      link.download = 'pengeluaranstok' + new Date().getTime() + '.xlsx';
                                      link.click();
                                    }
          ...
          $('#processingLoader').addClass('d-none')
                ...
                function getCekExport(params) {
                    ...
                    return new Promise((resolve, reject) => {
                        $.ajax({
                            url: `${apiUrl}pengeluaranstok/export?${params}`,
                            dataType: "JSON",
                            headers: {
                                Authorization: `Bearer ${accessToken}`
                            },
