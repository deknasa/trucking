<?php
if (isset($id)) { ?>
    <table id="marketingLookup<?= $id ?>" class="lookup-grid"></table>
<?php
} else { ?>
    <table id="marketingLookup" class="lookup-grid"></table>
<?php } ?>
...
<script>
  ...
  selector = $(`#marketingLookup{{ isset($id) ? $id : null }} `)
  ...
  selector.jqGrid({
    url: `{{ config('app.api_url') . 'marketing' }}`,
    mtype: "GET",
    styleUI: 'Bootstrap4',
    iconSet: 'fontAwesome',
    datatype: "json",
    ...
    loadBeforeSend: function(jqXHR) {
      ...
      if (detectDeviceType() == 'mobile') {
        ...
        $(`#gbox_${idTop}`).find('.ui-marketingdata-top').css({
          "height": "1px",
        })
      } else {
        var title = '{{ $title ?? '
