...
<script>
  ...
  $(document).on('click', '#btnReloadLookup', function(event) {
    loadDataHeaderLookup('penerimaanheader', 'penerimaanHeaderLookup')
  })
  $('#penerimaanHeaderLookup').jqGrid({
      ...
      idPrefix: 'penerimaanHeaderLookup',
      datatype: "json",
      colModel: [{
          label: 'ID',
          name: 'id',
      ...
      sortorder: 'asc',
      page: 1,
      // pager: $('#penerimaanHeaderLookupPager'),
      viewrecords: true,
      prmNames: {
        sort: 'sortIndex',
        order: 'sortOrder',
      ...
      loadComplete: function(data) {
        ...
        if (detectDeviceType() == 'desktop') {
          ...
          if (indexRow - 1 > $('#penerimaanHeaderLookup').getGridParam().reccount) {
            indexRow = $('#penerimaanHeaderLookup').getGridParam().reccount - 1
          }

          if (triggerClick) {
            if (id != '') {
              indexRow = parseInt($('#jqGrid').jqGrid('getInd', id)) - 1
              $(`#penerimaanHeaderLookup [id="${$('#penerimaanHeaderLookup').getDataIDs()[indexRow]}"]`).click()
              id = ''
            } else if (indexRow != undefined) {
              $(`#penerimaanHeaderLookup [id="${$('#penerimaanHeaderLookup').getDataIDs()[indexRow]}"]`).click()
            }

            if ($('#penerimaanHeaderLookup').getDataIDs()[indexRow] == undefined) {
              $(`#penerimaanHeaderLookup [id="` + $('#penerimaanHeaderLookup').getDataIDs()[0] + `"]`).click()
            }

            triggerClick = false
          } else {
            $('#penerimaanHeaderLookup').setSelection($('#penerimaanHeaderLookup').getDataIDs()[indexRow])
          }
        }
        ...
        $(this).setGridWidth($('#lookupPenerimaanHeader').prev().width())
        setHighlight($(this))
      }
    })
    ...
    .jqGrid('filterToolbar', {
      ...
      beforeSearch: function() {
        ...
        clearGlobalSearch($('#penerimaanHeaderLookup'))
      },
    })
    .customPager()
