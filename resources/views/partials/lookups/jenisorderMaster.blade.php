<?php
if (isset($id)) { ?>
<table id="jenisOrderLookup<?= $id ?>" class="lookup-grid"></table>
<?php
} else { ?>
...
<script>
    ...
    selector = $(`#jenisOrderLookup{{ isset($id) ? $id : null }} `)
                    ...
                    dataInit: function(element) {
                        $(element).select2({
                            ...
                            ajax: {
                                ...
                                beforeSend: () => {
                                    // clear options
                                    $(element).data('select2').$results.children().filter((index,
                                        element) => {
    ...
    selector.jqGrid({
            url: `{!! $url ?? config('app.api_url') . 'jenisorder' !!}`,
            mtype: "GET",
            styleUI: 'Bootstrap4',
            iconSet: 'fontAwesome',
            datatype: "json",
