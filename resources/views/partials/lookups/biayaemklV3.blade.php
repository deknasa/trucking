<?php
if (isset($id)) { ?>
    <table id="biayaemklLookup<?= $id ?>" class="lookup-grid"></table>
<?php
} else { ?>
    <table id="biayaemklLookup" class="lookup-grid"></table>
<?php } ?>
...
<script>
  ...
  selector = $(`#biayaemklLookup{{ isset($id) ? $id : null }} `)
  ...
  selector.jqGrid({
    url: `{{ config('app.api_url') . 'biayaemkl' }}`,
    mtype: "GET",
    styleUI: 'Bootstrap4',
    iconSet: 'fontAwesome',
    datatype: "json",
