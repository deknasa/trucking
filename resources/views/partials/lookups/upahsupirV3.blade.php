<?php
    if (isset($id)) { ?>
<table id="upahSupirLookup<?= $id ?>" class="lookup-grid"></table>
<?php
    } else { ?>
...
<script>
    ...
    selector = $(`#upahSupirLookup{{ isset($id) ? $id : null }} `)
    ...
    selector.jqGrid({
      url: `{!! $url ?? config('app.api_url').'upahsupir' !!}`,
      mtype: "GET",
      styleUI: 'Bootstrap4',
      iconSet: 'fontAwesome',
      datatype: "json",
      ...
      serializeGridData: function(postData) {
        ...
        loadComplete: function(data) {
            ...
            if (data.data.length === 0) {
                ...
                $('#upahSupirGrid').each((index, element) => {
