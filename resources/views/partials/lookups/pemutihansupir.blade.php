...
<script>
  ...
  $(document).on('click', '#btnReloadLookup', function(event) {
    loadDataHeaderLookup('pemutihansupir', 'pemutihanSupirLookup')
  })
  $('#pemutihanSupirLookup').jqGrid({
      url: `{{ config('app.api_url') . 'pemutihansupir' }}`,
      mtype: "GET",
      styleUI: 'Bootstrap4',
      iconSet: 'fontAwesome',
      datatype: "json",
      ...
      loadComplete: function(data) {
        ...
        if (detectDeviceType() == 'desktop') {
          ...
          if (indexRow - 1 > $('#pemutihanSupirLookup').getGridParam().reccount) {
            indexRow = $('#pemutihanSupirLookup').getGridParam().reccount - 1
          }

          if (triggerClick) {
            if (id != '') {
              indexRow = parseInt($('#jqGrid').jqGrid('getInd', id)) - 1
              $(`#pemutihanSupirLookup [id="${$('#pemutihanSupirLookup').getDataIDs()[indexRow]}"]`).click()
              id = ''
            } else if (indexRow != undefined) {
              $(`#pemutihanSupirLookup [id="${$('#pemutihanSupirLookup').getDataIDs()[indexRow]}"]`).click()
            }

            if ($('#pemutihanSupirLookup').getDataIDs()[indexRow] == undefined) {
              $(`#pemutihanSupirLookup [id="` + $('#pemutihanSupirLookup').getDataIDs()[0] + `"]`).click()
            }

            triggerClick = false
          } else {
            $('#pemutihanSupirLookup').setSelection($('#pemutihanSupirLookup').getDataIDs()[indexRow])
          }
        }
