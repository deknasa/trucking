<?php
if (isset($id)) { ?>
    <table id="jobemklLookup<?= $id ?>" class="lookup-grid"></table>
<?php
} else { ?>
    <table id="jobemklLookup" class="lookup-grid"></table>
<?php } ?>
...
<script>
  ...
  selector = $(`#jobemklLookup{{ isset($id) ? $id : null }} `)
  ...
  selector.jqGrid({
    url: `{{ config('app.api_url') . 'jobemkl' }}`,
    mtype: "GET",
    styleUI: 'Bootstrap4',
    iconSet: 'fontAwesome',
    datatype: "json",
