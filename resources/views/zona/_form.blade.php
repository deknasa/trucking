          ...
          <div class="card-body">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="id" class="form-control" value="{{ $zona['id'] ?? '' }}" readonly>
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="keterangan" class="form-control" value="{{ $zona['keterangan'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
              ...
              <div class="col-12 col-md-10">
                <select name="statusaktif" class="form-control select2bs4">
                        ...
                        <?php foreach ($combo['statusaktif'] as $key => $item) { 
                            $selected = @$zona['statusaktif'] == $item['id'] ? "selected" : ""
                        ?>
...
<script>
  ...
  let fieldLengthUrl = "{{ route('zona.field_length') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('zona.store') }}"
  let method = "POST"
  let csrfToken = "{{ csrf_token() }}"

  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('zona.update', $zona['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('zona.destroy', $zona['id']) }}"
    method = "DELETE"
  <?php endif; ?>
