...
<script>
    ...
    function selectAllRows() {
        $.ajax({
            ...
            success: (response) => {
                selectedRows = response.data.map((zona) => zona.id)
                $('#jqGrid').trigger('reloadGrid')
            }
        })
    }
    ...
    $(document).ready(function() {
            ...
            .customPager({
                ...
                modalBtnList: [{
                    ...
                    item: [{
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('zona', 'approvalaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('zona', 'approvalaktif') }}`) {
                                    approvalAktif('zona')
                        ...
                        },
                        {
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('zona', 'approvalnonaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('zona', 'approvalnonaktif') }}`) {
                                    approvalNonAktif('zona')
                                }
                            }
                        },
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('zona', 'report') }}`) {
        ...
        $('#formRange').submit(event => {
            ...
            getCekExport(params).then((response) => {
                    if ($('#rangeModal').data('action') == 'export') {
                                ...
                                if (xhr.status === 200) {
                                    if (response !== undefined) {
                                        var blob = new Blob([response], {
                                            type: 'zona/vnd.ms-excel'
                                        });
                                        var link = document.createElement('a');
                                        link.href = window.URL.createObjectURL(blob);
                                        link.download = 'zona' + new Date().getTime() + '.xlsx';
                                        link.click();
                                    }
                        ...
                        window.open(`{{ route('zona.report') }}?${params}`)
                        submitButton.prop('disabled', false)
                        $('#processingLoader').addClass('d-none')
                        $('#rangeModal').modal('hide')
                    }
