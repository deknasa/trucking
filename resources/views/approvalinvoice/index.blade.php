                ...
                <form id="crudForm">
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-12 col-sm-9 col-md-10">
                                <select name="invoice" id="invoice" class="form-select select2bs4" style="width: 100%;">
...
<!-- Detail -->
@include('approvalinvoice._detail')

@push('scripts')
<script>
    let indexUrl = "{{ route('approvalinvoiceheader.index') }}"
    let indexRow = 0;
    let page = 0;
    ...
    $(document).ready(function() {
        ...
        initSelect2($('#crudForm').find('[name=invoice]'), false)
        ...
        $(document).on('click', '#btnReload', function(event) {
            ...
            $('#jqGrid').jqGrid('setGridParam', {
                postData: {
                    ...
                    invoice: $('#crudForm').find('[name=invoice]').val(),
                },
            }).trigger('reloadGrid');
    ...
    function approve() {
        ...
        $.ajax({
            url: `${apiUrl}approvalinvoiceheader`,
            method: 'POST',
            dataType: 'JSON',
            headers: {
            ...
            success: response => {
                ...
                $('#crudForm').find('[name=invoice]').val(data.invoice)
                selectedRows = []
            },
            error: error => {
                if (error.status === 422) {
    ...
    function grid() {
        ...
        $("#jqGrid").jqGrid({
                url: `{{ config('app.api_url') . 'approvalinvoiceheader' }}`,
                mtype: "GET",
                styleUI: 'Bootstrap4',
                iconSet: 'fontAwesome',
                postData: {
        ...
        if (!`{{ $myAuth->hasPermission('approvalinvoiceheader', 'store') }}`) {
    ...
    function showDefault(form) {
        $.ajax({
            url: `${apiUrl}approvalinvoiceheader/default`,
            method: 'GET',
            dataType: 'JSON',
            headers: {
                Authorization: `Bearer ${accessToken}`
    ...
    const setStatusInvoiceOptions = function(relatedForm) {
        // return new Promise((resolve, reject) => {
        // relatedForm.find('[name=approve]').empty()
        relatedForm.find('[name=invoice]').append(
        ...
        $.ajax({
            ...
            success: response => {
                ...
                response.data.forEach(statusApproval => {
                    let option = new Option(statusApproval.text, statusApproval.id)
                    relatedForm.find('[name=invoice]').append(option).trigger('change')
                });
            }
        })
        // })
