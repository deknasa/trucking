...
<script>
  ...
  let getUrl = "{{ route('jenistrado.get') }}"
  let indexRow = 0;
  let page = 0;
  ...
  $(document).ready(function() {
    ...
    $("#jqGrid").jqGrid({
      ...
      .navButtonAdd(pager, {
        ...
        onClickButton: function() {
          ...
          window.location.href = `{{ route('jenistrado.create') }}?sortname=${sortname}&sortorder=${sortorder}&limit=${limit}`
        }
      })
    ...
    if (!`{{ $myAuth->hasPermission('jenistrado', 'create') }}`) {
      $('#add').addClass('ui-disabled')
    }

    if (!`{{ $myAuth->hasPermission('jenistrado', 'edit') }}`) {
      $('#edit').addClass('ui-disabled')
    }

    if (!`{{ $myAuth->hasPermission('jenistrado', 'delete') }}`) {
