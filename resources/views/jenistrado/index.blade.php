...
<script>
    ...
    function selectAllRows() {
        $.ajax({
            ...
            success: (response) => {
                selectedRows = response.data.map((jenistrado) => jenistrado.id)
                $('#jqGrid').trigger('reloadGrid')
            }
        })
    }
    ...
    $(document).ready(function() {
            ...
            .customPager({
                ...
                modalBtnList: [{
                    ...
                    item: [{
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('jenistrado', 'approvalaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('jenistrado', 'approvalaktif') }}`) {
                                    approvalAktif('jenistrado')
                        ...
                        },
                        {
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('jenistrado', 'approvalnonaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('jenistrado', 'approvalnonaktif') }}`) {
                                    approvalNonAktif('jenistrado')
                                }
                            }
                        },
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('jenistrado', 'report') }}`) {
        ...
        $('#formRange').submit(event => {
            ...
            getCekExport(params).then((response) => {
                    if ($('#rangeModal').data('action') == 'export') {
                        $.ajax({
                            url: `${apiUrl}jenistrado/export?${params}`,
                            type: 'GET',
                                ...
                                if (xhr.status === 200) {
                                    if (response !== undefined) {
                                        var blob = new Blob([response], {
                                            type: 'jenistrado/vnd.ms-excel'
                                        });
                                        var link = document.createElement('a');
                                        link.href = window.URL.createObjectURL(blob);
                                        link.download = 'jenistrado' + new Date().getTime() + '.xlsx';
                                        link.click();
                                    }
                        ...
                        window.open(`{{ route('jenistrado.report') }}?${params}`)
                        submitButton.removeAttr('disabled')
                        $('#processingLoader').addClass('d-none')
                        $('#rangeModal').modal('hide')
                    }
