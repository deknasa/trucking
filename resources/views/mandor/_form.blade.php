      ...
      <div class="card card-primary">
        ...
        <form action="" method="post">
          <div class="card-body">
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="id" class="form-control" value="{{ $mandor['id'] ?? '' }}" readonly>
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="namamandor" class="form-control" value="{{ $mandor['namamandor'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
              <div class="col-12 col-sm-3 col-md-2 col-form-label">
              ...
              <div class="col-12 col-sm-9 col-md-10">
                <input type="text" name="keterangan" class="form-control" value="{{ $mandor['keterangan'] ?? '' }}">
              </div>
            </div>
            <div class="row form-group">
              ...
              <div class="col-12 col-md-10">
                <select name="statusaktif" class="form-control select2bs4">
                        ...
                        <?php foreach ($combo['statusaktif'] as $key => $item) { 
                            $selected = @$mandor['statusaktif'] == $item['id'] ? "selected" : ""
                        ?>
          ...
          <div class="card-footer">
            ...
            <a href="{{ route('mandor.index') }}" class="btn btn-danger">
...
@push('scripts')
<script>
  let indexUrl = "{{ route('mandor.index') }}"
  let fieldLengthUrl = "{{ route('mandor.field_length') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('mandor.store') }}"
  let method = "POST"
  let csrfToken = "{{ csrf_token() }}"

  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('mandor.update', $mandor['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('mandor.destroy', $mandor['id']) }}"
    method = "DELETE"
  <?php endif; ?>
