...
<script>
    ...
    function selectAllRows() {
        $.ajax({
            ...
            success: (response) => {
                selectedRows = response.data.map((mandor) => mandor.id)
                $('#jqGrid').trigger('reloadGrid')
            }
        })
    }
    ...
    $(document).ready(function() {
            ...
            .customPager({
                ...
                modalBtnList: [{
                    ...
                    item: [{
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('mandor', 'approvalaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('mandor', 'approvalaktif') }}`) {
                                    approvalAktif('mandor')
                                }
                            }
                        },
                        {
                            ...
                            hidden: (!`{{ $myAuth->hasPermission('mandor', 'approvalnonaktif') }}`),
                            onClick: () => {
                                if (`{{ $myAuth->hasPermission('mandor', 'approvalnonaktif') }}`) {
                                    approvalNonAktif('mandor')
                                }
                            }
                        },
        ...
        function permission() {
            ...
            if (!`{{ $myAuth->hasPermission('mandor', 'report') }}`) {
        ...
        $('#formRange').submit(event => {
            ...
            getCekExport(params).then((response) => {
                    if ($('#rangeModal').data('action') == 'export') {
                        $.ajax({
                            url: `${apiUrl}mandor/export?${params}`,
                            type: 'GET',
                                ...
                                if (xhr.status === 200) {
                                    if (response !== undefined) {
                                        var blob = new Blob([response], {
                                            type: 'mandor/vnd.ms-excel'
                                        });
                                        var link = document.createElement('a');
                                        link.href = window.URL.createObjectURL(blob);
                                        link.download = 'mandor' + new Date().getTime() + '.xlsx';
                                        link.click();
                                    }
                        ...
                        window.open(`{{ route('mandor.report') }}?${params}`)
                        submitButton.removeAttr('disabled')
                        $('#processingLoader').addClass('d-none')
                        $('#rangeModal').modal('hide')
                    }
