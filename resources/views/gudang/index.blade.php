        ...
        <script>
            ...
            function selectAllRows() {
                $.ajax({
                    ...
                    success: (response) => {
                        selectedRows = response.data.map((gudang) => gudang.id)
                        $('#jqGrid').trigger('reloadGrid')
                    }
                })
            }
            ...
            $(document).ready(function() {
                ...
                if (!`{{ $myAuth->hasPermission('gudang', 'show') }}`) {
                ...
                if (!`{{ $myAuth->hasPermission('gudang', 'export') }}`) {
                ...
                $('#formRange').submit(event => {
                    ...
                    getCekExport(params).then((response) => {
                        if ($('#rangeModal').data('action') == 'export') {
                            $.ajax({
                                url: '{{ config('app.api_url') }}gudang/export?' + params,
                                type: 'GET',
                                    ...
                                    if (xhr.status === 200) {
                                        if (response !== undefined) {
                                            var blob = new Blob([response], {
                                                type: 'gudang/vnd.ms-excel'
                                            });
                                            var link = document.createElement('a');
                                            link.href = window.URL.createObjectURL(blob);
                                            link.download = 'gudang' + new Date().getTime() + '.xlsx';
                                            link.click();
                                        }
                            ...
                            window.open(`{{ route('gudang.report') }}?${params}`)
                            submitButton.removeAttr('disabled')
                            $('#processingLoader').addClass('d-none')
                            $('#rangeModal').modal('hide')
                        }
          ...
          $('#processingLoader').addClass('d-none')
                ...
                function getCekExport(params) {
                    ...
                    return new Promise((resolve, reject) => {
                        $.ajax({
                            url: `${apiUrl}gudang/export?${params}`,
                            dataType: "JSON",
                            headers: {
                                Authorization: `Bearer ${accessToken}`
                            },
