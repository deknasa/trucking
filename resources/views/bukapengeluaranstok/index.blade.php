...
@include('bukapengeluaranstok._modal')
...
<script>
  ...
  function selectAllRows() {
    $.ajax({
      ...
      success: (response) => {
        selectedRows = response.data.map((bukapengeluaranstok) => bukapengeluaranstok.id)
        $('#jqGrid').trigger('reloadGrid')
      }
    })
  }
  ...
  $(document).ready(function() {
          ...
          },
          {
            ...
            onClick: () => {
              ...
              if (selectedId == null || selectedId == '' || selectedId == undefined) {
                ...
                deleteBukaPengeluaranStok(selectedId)
              }
            }
          },
          {
    ...
    if (!`{{ $myAuth->hasPermission('bukapengeluaranstok', 'store') }}`) {
      $('#add').attr('disabled', 'disabled')
    }

    if (!`{{ $myAuth->hasPermission('bukapengeluaranstok', 'update') }}`) {
      $('#edit').attr('disabled', 'disabled')
    }

    if (!`{{ $myAuth->hasPermission('bukapengeluaranstok', 'destroy') }}`) {
    ...
    function multiUpdateTanggalBatas() {
      ...
      $.ajax({
        url: `${apiUrl}bukapengeluaranstok/updatetanggalbatas`,
        method: 'POST',
        dataType: 'JSON',
        headers: {
          Authorization: `Bearer ${accessToken}`
