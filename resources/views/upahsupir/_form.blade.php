<div class="container-fluid">
        ...
        <form action="" method="post">
          <div class="card-body">
                ...
                <div class="col-12 col-md-10">
                  <select name="kotadari_id" class="form-control select2bs4">
                    ...
                    <?php foreach ($combo['kota'] as $key => $item) {
                      $selected = @$upahsupir['kotadari_id'] == $item['id'] ? "selected" : ""
                    ?>
              ...
              <div class="row form-group">
                ...
                <div class="col-12 col-md-10">
                  <select name="kotasampai_id" class="form-control select2bs4">
                    ...
                    <?php foreach ($combo['kota'] as $key => $item) {
                      $selected = @$upahsupir['kotasampai_id'] == $item['id'] ? "selected" : ""
                    ?>
                      <option value="{{ $item['id'] }}" {{ $selected }}>{{ $item['keterangan'] }}</option>
                    <?php } ?>
                  </select>
                ...
                <div class="col-12 col-md-10">
                  <select name="zona_id" class="form-control select2bs4">
                    ...
                    <?php foreach ($combo['zona'] as $key => $item) {
                      $selected = @$upahsupir['zona_id'] == $item['id'] ? "selected" : ""
                    ?>
                      <option value="{{ $item['id'] }}" {{ $selected }}>{{ $item['keterangan'] }}</option>
                    <?php } ?>
                  </select>
            ...
            <div class="row form-group">
              ...
              <div class="col-12 col-md-10">
                <input type="text" name="jarak" class="form-control numbernoseparate" value="{{ $upahsupir['jarak'] ?? '' }}">
              </div>
            </div>
            ...
            <div class="row form-group">
                ...
                <div class="col-12 col-md-10">
                  <select name="statusaktif" class="form-control select2bs4">
                    ...
                    <?php foreach ($combo['statusaktif'] as $key => $item) {
                      $selected = @$upahsupir['statusaktif'] == $item['id'] ? "selected" : ""
                    ?>
              ...
              <div class="col-12 col-md-10">
                <input type="text" name="tglmulaiberlaku" class="form-control formatdate" value="{{ $upahsupir['tglmulaiberlaku'] ?? '' }}">
              </div>
            </div>
                ...
                <div class="col-12 col-md-10">
                  <select name="statusluarkota" class="form-control select2bs4">
                    ...
                    <?php foreach ($combo['statusluarkota'] as $key => $item) {
                      $selected = @$upahsupir['statusluarkota'] == $item['id'] ? "selected" : ""
                    ?>
                      <option value="{{ $item['id'] }}" {{ $selected }}>{{ $item['text'] }}</option>
                    <?php } ?>
                  </select>
                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-bindkeys">
                    ...
                    <tbody id="table_body" class="form-group">
                      @if (isset($upahsupir['upahsupir_rincian']))
                      @foreach($upahsupir['upahsupir_rincian'] as $upahsupirIndex => $d)
                      <tr id="row">
                        <td>
                          <div class="baris">{{ $upahsupirIndex+1 }}</div>
                        </td>
                        <td>
  ...
  <div class="card-footer">
    ...
    <a href="{{ route('upahsupir.index') }}" class="btn btn-danger">
...
<script>
  ...
  var baris = 1;
  @if(isset($upahsupir['upahsupirRincian']))
  baris = "{{count($upahsupir['upahsupirRincian'])}}";
  @endif
  ...
  let indexUrl = "{{ route('upahsupir.index') }}"
  let action = "{{ $action }}"
  let actionUrl = "{{ route('upahsupir.store') }}"
  let method = "POST"
  ...
  /* Set action url */
  <?php if ($action == 'edit') : ?>
    actionUrl = "{{ route('upahsupir.update', $upahsupir['id']) }}"
    method = "PATCH"
  <?php elseif ($action == 'delete') : ?>
    actionUrl = "{{ route('upahsupir.destroy', $upahsupir['id']) }}"
    method = "DELETE"
  <?php endif; ?>
